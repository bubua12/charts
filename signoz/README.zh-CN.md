...
## 1、安装
下载 `signoz` 目录到指定文件夹下，修改 `storageClass` 为 k8s 里的 sc

### 1.1 部署 signoz 平台
一般部署在 `platform` 命名空间里，这里 `ns` 写 `platform` （没有则自动创建命名空间）
```shell
helm install -n platform --create-namespace "platform" signoz/signoz
```
...
## 2、相关配置修改
## 2.1 容器副本分片等配置
根据集群等硬件资源情况修改，项目里clickhouse、ck等均为默认副本分片数。

## 2.2 FAQs
### 1.容器一直初始化
类似 `migrator` 的 pod 内部容器报错日志如下，连接 clickhouse 失败，或者 ck 内部容器日志提示授权校验报错。

```log
Authentication failed: password is incorrect, or there is no user with such name.
```

可以考虑是 ck 允许的连接IP来源的限制，参考 values.yaml 文件的如下内容（大概在193行）
```yaml
